                ...
                <template v-for="(item, index) in dates">
                    <div class="slider"
                        ...
                        <div class="day" v-for="day in getDaies(item.date)"> 
                            <div  v-if="(day.date < maxdate && day.date > mindate)"
                            ...
                            <div  v-if="!(day.date < maxdate && day.date > mindate)" 
								:style="buildDateStyle(day.isToday, day.isDay)">    
                                {{day.week}}<br><strong  class="diable"  disabled="true">{{day.date.split('-')[2]}}</strong>
                            </div>
                        </div>
                    </div>
                </template>
...
export default {
    ...
    props: {
        ...
        maxdate: {
            type: String,
            default: moment().add(100, 'd').format('YYYY-MM-DD')
        }
    },
    data () {
        return {
    ...
    methods: {
        ...
        */
        ...
        touchmoveHandle (event) {
            // day.date < maxdate && day.date > mindate
            let vm = this,
                touch = event.touches[0]
            vm.isAnimation = true
